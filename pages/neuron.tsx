...
import { BiSearch, BiNetworkChart } from 'react-icons/bi'
...
export default function Neuron({ data }) {
...
export function GraphPage(props) {
  ...
  useEffect(() => {
		let url = ''
		if (view != null) {
			url = `/orgroam/view?id=${view}`
		} else {
			url = '/orgroam'
		}
		axios.get(url)
			.then((response) => {
				return updateGraphData(response.data)
		})
  }, [])
...
export const Graph = function (props: GraphProps) {
  ...
  const handleClick = (click: string, node: OrgRoamNode, event: any) => {
    switch (click) {
      ...
      case mouse.follow: {
				router.push(`/document?id=${node.id}`)
        break
      }
      case mouse.context: {
